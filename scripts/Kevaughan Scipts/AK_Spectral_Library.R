...
test<-alaskaSpecLib%>% group_by(PFT_2, Band) %>% mutate(Median_reflectance = median(Refl)) %>% ggplot(aes(Band,Median_reflectance))+geom_line(aes(color=PFT))
test+facet_wrap(facets= vars(PFT_2),  nrow=10, ncol=11)+theme(legend.position="none")
dev.off()
