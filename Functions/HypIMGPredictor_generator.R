...
test<-bandsRemove(HyperSpecImages[[2]])
colnames(test)
...
# test_case<-Func_Resamp(HyperSpecImages[[2]])
...
Func_VI<-function(VI){
  ...
  # Converts infinite values to 0s
  # converts NAs to zeros
  df[mapply(is.infinite,  df)] <- 0
  
  return(df)
}# Function ends 

# Test your data below
# test_case<-Func_VI(HyperSpecImages[[2]])
