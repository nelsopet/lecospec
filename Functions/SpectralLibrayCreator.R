...
SpectralLibrayCreator<-function(filename,out_file,Classif_Model,datatype,extension){
  ...
  if(datatype == "raster"){
    ...
    List_of_PredLayers<-lapply(1:length(list_of_Tiles), function(i){
      ...
      if(!file.exists(out_tif2)){
        ...
        DfofRas[-1:-2][DfofRas[-1:-2] > 1.5] <- 0
        DfofRas[-1:-2][DfofRas[-1:-2] < 0  ] <- 0
        ...
        RastoDF<-rasterFromXYZ(New_df,crs = "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0" )
        
        RastoDF<-raster::resample(RastoDF,RasterBrick,method = "ngb")
        ...
        Predicted_layer<-raster::predict(RastoDF,Model,na.rm = TRUE)
        ...
        writeRaster(Predicted_layer,filename = paste0(SubFolder,"/B_001_",basename(filename),"_Tile",i,"_PredLayer.tif")
                    ,overwrite = T)
