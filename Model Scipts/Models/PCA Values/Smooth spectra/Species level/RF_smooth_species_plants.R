...
training_plants<-pca_smooth_plants[index,]
testing_plants<-pca_smooth_plants[-index,]

###random forest model for (plant and abiotic scans)
rf_smooth_plants<-randomForest(PFT_2~.,data=training_plants,mtry=5,ntree=2001,importance=TRUE)
print(rf_smooth_plants)
result_smooth_plants<-data.frame(testing_plants$PFT_2,predict(rf_smooth_plants,testing_plants,type = "response"))
