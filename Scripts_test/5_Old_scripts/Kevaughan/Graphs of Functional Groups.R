...
alaskaSpecLib_Lichen    <-read.csv("Outputs/1_Field_spec/1_processing//alaskaSpecLib_Lichen.csv"    ,check.names = F)
alaskaSpecLib_Tree      <-read.csv("Outputs/1_Field_spec/1_processing//alaskaSpecLib_Tree.csv"      ,check.names = F)
alaskaSpecLib_Dwarfshrub<-read.csv("Outputs/1_Field_spec/1_processing//alaskaSpecLib_Dwarfshrub.csv",check.names = F)
alaskaSpecLib_shrub     <-read.csv("Outputs/1_Field_spec/1_processing//alaskaSpecLib_shrub.csv"     ,check.names = F)
alaskaSpecLib_Moss      <-read.csv("Outputs/1_Field_spec/1_processing//alaskaSpecLib_Moss.csv"      ,check.names = F)
alaskaSpecLib_Graminoid <-read.csv("Outputs/1_Field_spec/1_processing//alaskaSpecLib_Graminoid.csv" ,check.names = F)
alaskaSpecLib_Forb      <-read.csv("Outputs/1_Field_spec/1_processing//alaskaSpecLib_Forb.csv"      ,check.names = F)
...
##Lichen
jpeg("Lichen.jpg", units="px", height = 1400, width=2400, res=350)
alaskaSpecLib_Lichen%>%
  group_by(Species, Wavelength) %>% mutate(Median_Reflectance = median(Reflectance)) %>% 
  ggplot(aes(Wavelength,Median_Reflectance))+geom_line(aes(color=Species))+
  theme(panel.background = element_rect(fill = "white", colour = "grey50"))+labs(title="Lichen signatures")
dev.off()
