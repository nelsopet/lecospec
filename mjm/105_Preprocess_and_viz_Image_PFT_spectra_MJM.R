...
PFT_IMG_SPEC_wSite <- PFT_IMG_SPEC_clean %>%
  mutate(site = str_sub(UID, 1, 10)) %>%
  select(site, everything())
sites <- PFT_IMG_SPEC_wSite %>%
  group_by(site) %>%
  tally()

site_pfts <- PFT_IMG_SPEC_wSite %>%
  group_by(site, FncGrp1) %>%
  tally()

# PFT_IMG_SPEC_wSite %>% dplyr::select(site, FncGrp1)

table(PFT_IMG_SPEC_wSite %>% dplyr::select(site, FncGrp1))
...
PFT_IMG_SPEC_wSite %>% 
  ...
  dplyr::select(site, Functional_group1, PFT) %>% 
  unique() %>% 
  ungroup() %>% 
  group_by(Functional_group1) %>%
  tally() %>% 
  dplyr::rename(species_count = n) %>%
inner_join(PFT_IMG_SPEC_wSite, by=c("Functional_group1"="FncGrp1")) %>% 
  #columnwise_robust_scale(ignore_cols = names_ignore) %>%
  #standardize_df(ignore_cols = names_ignore) %>%

  group_by(Functional_group1, site) %>% 
  ...
  group_by(Functional_group1_wN, Functional_group1,site, Wavelength) %>%  
...
ggplot(PFT_IMG_SPEC_clean_tall, aes(Wavelength, Median_Reflectance,
                                    # group = c(Functional_group1, site), 
                                    color=site, fill=site), scales = "fixed")+
  labs(title = c("Image Spectra by Site and PFT"), y="Reflectance")+
  ...
  geom_ribbon(aes(Wavelength, ymin = Pct_12_5_Reflectance, ymax = Pct_87_5_Reflectance), alpha = 0.3, color=NA) +
  geom_ribbon(aes(Wavelength, ymin = Lower_Reflectance, ymax = Upper_Reflectance), alpha = 0.2, color=NA) +
  geom_line(aes(Wavelength, Median_Reflectance),size = 1)+  
  # facet_grid(Functional_group1 ~ site, scales = "free_y")#, ncol = 3) 
  facet_wrap(Functional_group1 ~ ., scales = "free_y")#, ncol = 3) 
# dev.off()
ggsave("Output/Fnc_grp1_spectral_profiles_ImageSpectra_by_site.png", height = 10, width = 16, dpi = 100)
