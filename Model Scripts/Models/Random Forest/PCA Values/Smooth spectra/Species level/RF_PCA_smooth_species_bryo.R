...
###extract error rate
error_rf_PCA_smooth_bryo<-as.data.frame(rf_smooth_bryo$err.rate[2001,1])
names(error_rf_PCA_smooth_bryo)[1]<-"smooth_all_bands"
error_rf_PCA_smooth_5nm_bryo<-as.data.frame(rf_smooth_5nm_bryo$err.rate[2001,1])
names(error_rf_PCA_smooth_5nm_bryo)[1]<-"smooth_5nm"
error_rf_PCA_smooth_10nm_bryo<-as.data.frame(rf_smooth_10nm_bryo$err.rate[2001,1])
names(error_rf_PCA_smooth_10nm_bryo)[1]<-"smooth_10nm"
error_rf_PCA_smooth_50nm_bryo<-as.data.frame(rf_smooth_50nm_bryo$err.rate[2001,1])
names(error_rf_PCA_smooth_50nm_bryo)[1]<-"smooth_50nm"
error_rf_PCA_smooth_100nm_bryo<-as.data.frame(rf_smooth_100nm_bryo$err.rate[2001,1])
names(error_rf_PCA_smooth_100nm_bryo)[1]<-"smooth_100nm"

###Make data frame from error rate
error_rate_PCA_smooth_bryo<-cbind(error_rf_PCA_smooth_bryo,
                              error_rf_PCA_smooth_5nm_bryo,
                              error_rf_PCA_smooth_10nm_bryo,
                              error_rf_PCA_smooth_50nm_bryo,
                              error_rf_PCA_smooth_100nm_bryo)
error_rate_PCA_smooth_bryo$category<-"Bryophytes"

##write to folder
write.csv(error_rate_PCA_smooth_bryo,"/Alaska_Spectral_Library/Models/Error Rates/PCA/error_rate_PCA_smooth_bryo.csv",row.names = F)
