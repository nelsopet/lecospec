...
###extract error rate
error_rf_PCA_smooth_lichen<-as.data.frame(rf_smooth_lichen$err.rate[2001,1])
names(error_rf_PCA_smooth_lichen)[1]<-"smooth_all_bands"
error_rf_PCA_smooth_5nm_lichen<-as.data.frame(rf_smooth_5nm_lichen$err.rate[2001,1])
names(error_rf_PCA_smooth_5nm_lichen)[1]<-"smooth_5nm"
error_rf_PCA_smooth_10nm_lichen<-as.data.frame(rf_smooth_10nm_lichen$err.rate[2001,1])
names(error_rf_PCA_smooth_10nm_lichen)[1]<-"smooth_10nm"
error_rf_PCA_smooth_50nm_lichen<-as.data.frame(rf_smooth_50nm_lichen$err.rate[2001,1])
names(error_rf_PCA_smooth_50nm_lichen)[1]<-"smooth_50nm"
error_rf_PCA_smooth_100nm_lichen<-as.data.frame(rf_smooth_100nm_lichen$err.rate[2001,1])
names(error_rf_PCA_smooth_100nm_lichen)[1]<-"smooth_100nm"

###Make data frame from error rate
error_rate_PCA_smooth_lichen<-cbind(error_rf_PCA_smooth_lichen,
                                error_rf_PCA_smooth_5nm_lichen,
                                error_rf_PCA_smooth_10nm_lichen,
                                error_rf_PCA_smooth_50nm_lichen,
                                error_rf_PCA_smooth_100nm_lichen)
error_rate_PCA_smooth_lichen$category<-"Lichen"

##write to folder
write.csv(error_rate_PCA_smooth_lichen,"/Alaska_Spectral_Library/Models/Error Rates/PCA/error_rate_PCA_smooth_lichen.csv",row.names = F)
