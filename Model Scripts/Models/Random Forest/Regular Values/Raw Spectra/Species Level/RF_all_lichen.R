...
###extract error rate
error_rf_lichen<-as.data.frame(rf_lichen$err.rate[2001,1])
names(error_rf_lichen)[1]<-"all_bands"
error_rf_5nm_lichen<-as.data.frame(rf_5nm_lichen$err.rate[2001,1])
names(error_rf_5nm_lichen)[1]<-"5nm"
error_rf_10nm_lichen<-as.data.frame(rf_10nm_lichen$err.rate[2001,1])
names(error_rf_10nm_lichen)[1]<-"10nm"
error_rf_50nm_lichen<-as.data.frame(rf_50nm_lichen$err.rate[2001,1])
names(error_rf_50nm_lichen)[1]<-"50nm"
error_rf_100nm_lichen<-as.data.frame(rf_100nm_lichen$err.rate[2001,1])
names(error_rf_100nm_lichen)[1]<-"100nm"

###Make data frame from error rate
error_rate_lichen<-cbind(error_rf_lichen,
                         error_rf_5nm_lichen,
                         error_rf_10nm_lichen,
                         error_rf_50nm_lichen,
                         error_rf_100nm_lichen)
error_rate_lichen$category<-"Lichen"

##write to folder
write.csv(error_rate_lichen,"/Alaska_Spectral_Library/Models/Error Rates/Regular/error_rate_lichen.csv",row.names = F)