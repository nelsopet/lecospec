...
################################Model all bands########################################
##PCA (all bands)
pca_plants<- prcomp(alaskaSpecLib_smooth_plants[,-2152], retx=TRUE, center=TRUE, scale=TRUE)
expl.var_plants <- round(pca_plants$sdev^2/sum(pca_plants$sdev^2)*100)
pc6<-c(1:6)####fist 6 PC's
names_species<-as.data.frame(as.character(alaskaSpecLib_smooth_plants$PFT_3))
pca_plants<-as.data.frame(cbind(names_species,pca_plants$x[,pc6]))
names(pca_plants)[1]<-paste("PFT_3")
pca_plants$PFT_3<-as.factor(pca_plants$PFT_3)
...
rf_smooth_plants<-randomForest(PFT_3~.,data=training_plants,mtry=5,ntree=2001,importance=TRUE)
print(rf_smooth_plants)
result_smooth_plants<-data.frame(testing_plants$PFT_3,predict(rf_smooth_plants,testing_plants,type = "response"))
...
pc6<-c(1:6)####fist 6 PC's
names_species<-as.data.frame(as.character(alaskaSpecLib_smooth_5nm_plants$PFT_3))
pca_smooth_5nm_plants<-as.data.frame(cbind(names_species,pca_smooth_5nm_plants$x[,pc6]))
names(pca_smooth_5nm_plants)[1]<-paste("PFT_3")
pca_smooth_5nm_plants$PFT_3<-as.factor(pca_smooth_5nm_plants$PFT_3)
...
pc6<-c(1:6)####fist 6 PC's
names_species<-as.data.frame(as.character(alaskaSpecLib_smooth_10nm_plants$PFT_3))
pca_smooth_10nm_plants<-as.data.frame(cbind(names_species,pca_smooth_10nm_plants$x[,pc6]))
names(pca_smooth_10nm_plants)[1]<-paste("PFT_3")
pca_smooth_10nm_plants$PFT_3<-as.factor(pca_smooth_10nm_plants$PFT_3)
...
pc6<-c(1:6)####fist 6 PC's
names_species<-as.data.frame(as.character(alaskaSpecLib_smooth_50nm_plants$PFT_3))
pca_smooth_50nm_plants<-as.data.frame(cbind(names_species,pca_smooth_50nm_plants$x[,pc6]))
names(pca_smooth_50nm_plants)[1]<-paste("PFT_3")
pca_smooth_50nm_plants$PFT_3<-as.factor(pca_smooth_50nm_plants$PFT_3)
...
pc6<-c(1:6)####fist 6 PC's
names_species<-as.data.frame(as.character(alaskaSpecLib_smooth_100nm_plants$PFT_3))
pca_smooth_100nm_plants<-as.data.frame(cbind(names_species,pca_smooth_100nm_plants$x[,pc6]))
names(pca_smooth_100nm_plants)[1]<-paste("PFT_3")
pca_smooth_100nm_plants$PFT_3<-as.factor(pca_smooth_100nm_plants$PFT_3)
...
error_rate_PCA_smooth_gen_life_form<-cbind(error_rf_PCA_smooth_plants,
...
error_rate_PCA_smooth_gen_life_form$category<-"Courser_levels"

##write to folder
write.csv(error_rate_PCA_smooth_gen_life_form,"/Alaska_Spectral_Library/Models/Error Rates/PCA/error_rate_PCA_smooth_gen_life_form.csv",row.names = F)
