...
##Reads in Imagery
EightMile_clipped_AV<-brick("Original_data/Test_imagery_AVIRIS/EightMile_clipped_AV")

##Marks raster as unrotated
EightMile_clipped_AV@rotated<-FALSE

##Converts to a dataframe
EightMile_clipped_AV<-rasterToPoints(EightMile_clipped_AV)%>% as.data.frame()
...
colnames(EightMile_clipped_AV)[-1:-2]<-AV_wv

##Now lets check the range of the values in the image
test=lapply(EightMile_clipped_AV[,-1:-2],range)%>%as.data.frame%>%t()%>%as.data.frame
#test%>%View()
test%>%lapply(range) ### There are negative values here
EightMile_clipped_AV%>%subset(`381.87`<0)%>%nrow()
...
EightMile_clipped_AV_matrix <-as.matrix(EightMile_clipped_AV [-1:-2])

##lets check the column attributes to see if any weird values were introduced
EightMile_clipped_AV_matrix %>%min()##values are fine you may proceed, i.e no negative values or values grater than 2,you'll ned to check min values using the function min()
...
EightMile_clipped_AV_speclib <-speclib(EightMile_clipped_AV_matrix ,AV_wv)
...
EightMile_clipped_AV_VIs <-vegindex(EightMile_clipped_AV_speclib ,index=VIs)

##lets do a logical test on EightMile_clipped_AV_VIs to see if strange values exist
test3<-lapply(EightMile_clipped_AV_VIs,range)%>%as.data.frame%>%t()%>%as.data.frame
...
EightMile_clipped_AV_VIs<-EightMile_clipped_AV_VIs%>%dplyr::select(-CAI,-Datt7,-Datt8,-DWSI1,-DWSI2,-DWSI3,-DWSI5,-LWVI1,-LWVI2,-MSI
                                                                 ,-NDLI,-NDNI,-NDWI,-PWI,-SRWI,-'SWIR FI',-'SWIR LI',-'SWIR SI',-'SWIR VI')

##we need to combine the other columns with our new VI variables
EightMile_clipped_AV_VIs <-cbind(EightMile_clipped_AV [1:2],EightMile_clipped_AV_VIs )
...
colnames(EightMile_clipped_AV_VIs)[-1:-2]<-newcolnames

#lets do a logical test again to the image
test6<-lapply(EightMile_clipped_AV_VIs[-1:-7], range)%>%as.data.frame%>%t()%>%as.data.frame()
...
write.csv(EightMile_clipped_AV_VIs         ,"Outputs/3_AV_Imagery/1_Processing/EightMile_clipped_AV_VIs.csv"         ,row.names = FALSE)
