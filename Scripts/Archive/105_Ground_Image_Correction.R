...
#
PFT_SPEC_GROUND_IMAGE %>%
  dplyr::filter(Source == "Image") %>%
  dplyr::filter(Wavelength == 450) %>%
  dplyr::select(Median_Reflectance) %>%
  unique()
...
PFT_IMG_GROUND_CORR <- PFT_IMG_GROUND_CORR %>%
  group_by(Functional_group1) %>%
  mutate(
    Median_Diff = Median_Reflectance_Ground - Median_Reflectance_Image,
...
ground_cols_keep <- Cleaned_Speclib %>%
  dplyr::select(X:X2500) %>%
  colnames()
...
P1 <- Cleaned_Speclib %>%
  ...
  mutate(
    Wavelength = gsub("X", "", Wavelength),
    Reflectance = Reflectance / 100
  ) %>% # dplyr::select(Wavelength) %>% unique() %>% as.data.frame() %>% View()
