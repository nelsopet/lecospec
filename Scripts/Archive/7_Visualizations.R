...
# Fnc Grp 2 colors
coarse_colors <- createPalette(
  length(
    unique(
      SpecLib_derivs$Functional_group2)),
...
coarse_color_list <- SpecLib_derivs %>%
  dplyr::select(Functional_group2) %>%
  inner_join(
    coarse_colors,
    by = c("Functional_group2" = "FNC_grp1"),
    keep = FALSE
  )
...
species_colors <- createPalette(
  length(unique(SpecLib_derivs$Species_name)),
  c("#ff0000", "#00ff00", "#0000ff")
  ) %>%
  as.data.frame() %>%
  dplyr::rename(Color = ".") %>%
...
species_color_list <- SpecLib_derivs %>%
  ...
  inner_join(
    species_colors,
    by = c("Species_name" = "FNC_grp1"), keep = FALSE
  )
...
# fnc_grp1_color_list<-Veg_env %>% select(Functional_group2) %>% inner_join(fnc_grp1_colors, by=c("Functional_group2"="FNC_grp1"), keep=FALSE)
genera_color_list <- SpecLib_derivs %>%
  dplyr::select(Functional_group1) %>%
  inner_join(genera_colors, by = c("Functional_group1" = "FNC_grp1"), keep = FALSE)
...
fnc_grp2_colors <- createPalette(length(unique(Veg_env$Functional_group2)), c("#ff0000", "#00ff00", "#0000ff")) %>%
  ...
  mutate(ColorNum = seq(1:length(unique(Veg_env$Functional_group2))))
# fnc_grp1_color_list<-Veg_env %>% select(Functional_group2) %>% inner_join(fnc_grp1_colors, by=c("Functional_group2"="FNC_grp1"), keep=FALSE)
fnc_grp1_color_list <- Veg_env %>%
  dplyr::select(Code_name) %>%
  inner_join(fnc_grp1_colors, by = c("Code_name" = "FNC_grp1"), keep = FALSE)
fnc_grp2_color_list <- Veg_env %>%
  dplyr::select(Functional_group2) %>%
  inner_join(fnc_grp2_colors, by = c("Functional_group2" = "FNC_grp1"), keep = FALSE)
...
Lichen <- Cleaned_Speclib %>%
  filter(Functional_group2 == "Lichen") %>%
  select(-ScanID, -Code_name, -Functional_group1, -Functional_group2, -Area, -Species_name_Freq, -Functional_group1_Freq, -Functional_group2_Freq, -Species_name)

Lichen_env %>%
  group_by(Species_name, Functional_group1) %>%
  tally() %>%
  View()
...
fnc_grp1_color_list <- Lichen_env %>%
  select(Functional_group1) %>%
  inner_join(fnc_grp1_colors, by = c("Functional_group1" = "FNC_grp1"), keep = FALSE)
...
Lichen_Yellow <- Cleaned_Speclib %>%
  filter(Functional_group1 == "Lichen_Fruticose_Yellow") %>%
  select(-ScanID, -Code_name, -Functional_group1, -Functional_group2, -Area, -Species_name_Freq, -Functional_group1_Freq, -Functional_group2_Freq, -Species_name)
...
Lichen_Yellow_env <- Cleaned_Speclib %>%
  filter(Functional_group1 == "Lichen_Fruticose_Yellow") %>%
  select(ScanID, Code_name, Functional_group1, Functional_group2, Area, Species_name_Freq, Functional_group1_Freq, Functional_group2_Freq, Species_name)
...
fnc_grp1_color_list <- Lichen_Yellow_env %>%
  select(Species_name) %>%
  inner_join(fnc_grp1_colors, by = c("Species_name" = "FNC_grp1"), keep = FALSE)
...
Moss <- Cleaned_Speclib %>%
  filter(Functional_group2 == "Moss") %>%
  select(-ScanID, -Code_name, -Functional_group1, -Functional_group2, -Area, -Species_name_Freq, -Functional_group1_Freq, -Functional_group2_Freq, -Species_name)
...
################# Shrub
unique(Cleaned_Speclib$Functional_group2)
Shrub <- Cleaned_Speclib %>%
  filter(Functional_group2 == "Shrub") %>%
  dplyr::select(-ScanID, -Code_name, -Functional_group1, -Functional_group2, -Area, -Species_name_Freq, -Functional_group1_Freq, -Functional_group2_Freq, -Species_name)
Shrub_derivs <- SpecLib_derivs %>%
  filter(Functional_group2 == "Shrub") %>%
  dplyr::select(-ScanID, -Code_name, -Functional_group1, -Functional_group2, -Area, -Species_name_Freq, -Functional_group1_Freq, -Functional_group2_Freq, -Species_name)
...
Shrub_env <- Cleaned_Speclib %>%
  filter(Functional_group2 == "Shrub") %>%
  dplyr::select(ScanID, Code_name, Functional_group1, Functional_group2, Area, Species_name_Freq, Functional_group1_Freq, Functional_group2_Freq, Species_name)
Shrub_deriv_env <- SpecLib_derivs %>%
  filter(Functional_group2 == "Shrub") %>%
  dplyr::select(ScanID, Code_name, Functional_group1, Functional_group2, Area, Species_name_Freq, Functional_group1_Freq, Functional_group2_Freq, Species_name)
...
unique(Cleaned_Speclib$Functional_group1)
Shrub <- Cleaned_Speclib %>%
  filter(Functional_group1 == "Shrub_Salix") %>%
  select(-ScanID, -Code_name, -Functional_group1, -Functional_group2, -Area, -Species_name_Freq, -Functional_group1_Freq, -Functional_group2_Freq, -Species_name)
