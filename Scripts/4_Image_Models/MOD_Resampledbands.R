...
#Lets create a dataframe that shows the variable importance
feat_imp_df1 <- importance(rf_HDW_resampled_05nm)[1:15,] %>% 
  data.frame() %>% 
  mutate(feature = row.names(.)) 

#now let's 05nmualize that dataframe plot dataframe
ggplot(feat_imp_df1, aes(x = reorder(feature, MeanDecreaseGini), 
                        y = MeanDecreaseGini)) +
  ...
  labs(
    x     = "Feature",
    y     = "Importance",
    title = "Feature Importance: Randomforest_MOD bands every 5nm as predictors"
  )

#Lets create a dataframe that shows the importance of each variable
feat_imp_df2 <- importance(rf_HDW_resampled_10nm)[1:15,] %>% 
  data.frame() %>% 
  mutate(feature = row.names(.)) 
