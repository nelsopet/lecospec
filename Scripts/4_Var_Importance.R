...
rf_mod_ranger<-ranger(Classes ~ .,data = SpecLib,
                    num.trees = 1000,
                    local.importance = TRUE) # OOB prediction error:             26.33 %

rf_mod_randomforest<-randomForest(Classes ~ .,data = SpecLib
                    ,ntree=1000,importance=TRUE) # OOB prediction error 27.05%

# Lets look at the confusion matrix for each class
# Ranger
Ranger_confusionmatrix<-rf_mod_ranger$confusion.matrix%>%as.data.frame.matrix()
write.csv(Ranger_confusionmatrix,"Output/Ranger_confusionmatrix.csv")

# Random Forest
RandomForest_confusionmatrix<-rf_mod_randomforest$confusion%>%as.data.frame()
write.csv(RandomForest_confusionmatrix,"Output/RandomForest_confusionmatrix.csv")
...
# ImportantVarsFrame<-enframe(Modslist_cor[[10]]$variable.importance, 
...
rf_mod_randomforest<-randomForest(Classes ~ .,data = predictor_df_reduced
                                   ,ntree=1000,importance=TRUE) # OOB prediction error 23.43%
 
rf_mod_ranger<-ranger(Classes ~ .,data = predictor_df_reduced,
                       num.trees = 1000,
                       local.importance = TRUE) # OOB prediction error:             24.28 % 


# Creates a sequence of numbers that, represents the number of varibles to choose to build models
NoofVars<-c(seq(5,87,by = 5),"87")
...
Modslist<-lapply(1:length(NoofVars),function(x){
  ...
  rfNew<-randomForest(Classes ~ .,data = New_Speclib
...
save(A_30_Most_ImpVaribles, file = "Output/Best_Model25.rda")
# saves the model with the lowest error
save(A_87_Most_ImpVaribles, file = "Output/Best_Model99rf.rda")
