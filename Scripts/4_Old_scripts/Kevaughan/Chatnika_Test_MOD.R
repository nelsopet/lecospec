...
Chatnika_HDW<-brick("Original_data/Test_imagery_HDW/Chatnika_clipped_HDW_N")
Chatnika_HDW_df<-brick("Original_data/Test_imagery_HDW/Chatnika_clipped_HDW_N")%>%rasterToPoints()%>%as.data.frame()

Chatnika_data_HDW<-read.csv("Outputs/2_HDW_Imagery/1_processing/Chatnika_data_HDWN.csv")
...
write.csv(Chatnika_ConfusionMatrix,"Outputs/2_HDW_imagery/2_Models/Chatnika_ConfusionMatrix.csv")
...
Results_HDW<-cbind(Results_HDW,Chatnika_HDW_df[1:2]) %>% dplyr::select(predicted,x,y)
...
Chatnika_HDW_PercentCover<-Chatnika_HDW_pixeltotals[,2:7]/(Chatnika_HDW_pixeltotals[,1])*100
...
write.csv(Chatnika_HDW_PercentCover,"Outputs/2_HDW_imagery/2_Models/Chatnika_model_PercentCover.csv")
...
jpeg('Outputs/2_HDW_Imagery/2_Models/Chatnika Plot of VIs Model Prediction.jpg',width=1200, height=700)
plot(
  Chatnika_raster,
  legend = FALSE,
  axes=FALSE,
