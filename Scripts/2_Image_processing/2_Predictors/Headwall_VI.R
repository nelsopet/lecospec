...
test2%>%lapply(range)
test2<-test2%>%subset(V1<0)####Why am I getting negative values here?
...
##Creates dataframe with Vegitation indices as variables
...
test3%>%View()##There are columns where NaNs exist because the spectral range of the sensor is 400nm-100nm
              ##This means some Veg indices won't generate values because there aren't those bands present
...
##we need to combine tho other columns with our new VI variables
...
test5%>%View()###There are NaNs and infs, lets remove them
...
alaskaSpeclib_HDW_VIs<-subset(alaskaSpeclib_HDW_VIs, is.infinite(Datt)      ==F)
alaskaSpeclib_HDW_VIs<-subset(alaskaSpeclib_HDW_VIs, is.na      (MCARIOSAVI)==F)
alaskaSpeclib_HDW_VIs<-subset(alaskaSpeclib_HDW_VIs, is.infinite(PRI_norm)  ==F)
alaskaSpeclib_HDW_VIs<-subset(alaskaSpeclib_HDW_VIs, is.infinite(SIPI)      ==F)
alaskaSpeclib_HDW_VIs<-subset(alaskaSpeclib_HDW_VIs, is.na      (TCARIOSAVI)==F)

##Now lets equalize the scans per functional group
