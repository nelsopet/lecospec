...
set.seed(2017)
rf_HDW_EightMileTest <-randomForest(PFT_3~.,data=alaskaSpecLib_data_HDW  ,mtry=12,ntree=1001,localImp = TRUE)
##save(rf_HDW_EightMileTest, file = "Test_Outputs/2_HDW_Imagery/2_Models/rf_HDW_EightMileTest.rda")

plot(rf_HDW_EightMileTest)

##Lets save the confusion Matrix for these models
EightMileTest_ConfusionMatrix<-rf_HDW_EightMileTest$confusion%>%
  as.data.frame()%>%dplyr::select(class.error)%>%rename(VAR_ALL =class.error )
...
load("Test_Outputs/2_HDW_Imagery/2_Models/rf_HDW_EightMileTest.rda")
...
min_depth_frame <- min_depth_distribution(rf_HDW_EightMileTest)
head(min_depth_frame, n = 10)

##Now we can visualize this
#We will first obtain a plot of the distribution of minimal depth for the top ten variables
##This is based on the mean minimal deth 
a<-plot_min_depth_distribution(min_depth_frame, k= 10)
b<-plot_min_depth_distribution(min_depth_frame, k= 25)
c<-plot_min_depth_distribution(min_depth_frame, k= 50)

##Lets take the 25 most important variables and rebuild our models
var_namesa<-unique(a$data$variable)%>%
  as.character()
var_namesb<-unique(b$data$variable)%>%
  as.character()
var_namesc<-unique(c$data$variable)%>%
  as.character()

##Lets subsET our spectral library and imagery to have just those variables
alaskaSpecLib_data_NEWa<-alaskaSpecLib_data_HDW%>%
  dplyr::select(PFT_3,var_namesa)
EightMileTest_data_HDW_na<-EightMileTest_data_HDW%>%
  dplyr::select(x,y,var_namesa)

alaskaSpecLib_data_NEWb<-alaskaSpecLib_data_HDW%>%
  dplyr::select(PFT_3,var_namesb)
EightMileTest_data_HDW_nb<-EightMileTest_data_HDW%>%
  dplyr::select(x,y,var_namesb)

alaskaSpecLib_data_NEWc<-alaskaSpecLib_data_HDW%>%
  dplyr::select(PFT_3,var_namesc)
EightMileTest_data_HDW_nc<-EightMileTest_data_HDW%>%
  dplyr::select(x,y,var_namesc)

##Now we can rebuild our model
rf_HDW_EightMileTest_na <-randomForest(PFT_3~.,data=alaskaSpecLib_data_NEWa  ,mtry=3,ntree=1001,localImp = TRUE)
rf_HDW_EightMileTest_nb <-randomForest(PFT_3~.,data=alaskaSpecLib_data_NEWb  ,mtry=5,ntree=1001,localImp = TRUE)
rf_HDW_EightMileTest_nc <-randomForest(PFT_3~.,data=alaskaSpecLib_data_NEWc  ,mtry=7,ntree=1001,localImp = TRUE)


##Lets save the confusion Matrix for these models
EightMileTest_ConfusionMatrixa<-rf_HDW_EightMileTest_na$confusion%>%
  as.data.frame()%>%dplyr::select(class.error)%>%rename(VAR_10 =class.error )
EightMileTest_ConfusionMatrixb<-rf_HDW_EightMileTest_nb$confusion%>%
  as.data.frame()%>%dplyr::select(class.error)%>%rename(VAR_25 =class.error )
EightMileTest_ConfusionMatrixc<-rf_HDW_EightMileTest_nc$confusion%>%
  as.data.frame()%>%dplyr::select(class.error)%>%rename(VAR_50 =class.error )

##Lets combine these dataframes so we can compare them later
EightMileTest_ConfusionMatrix_all<-cbind(EightMileTest_ConfusionMatrix
                                        ,EightMileTest_ConfusionMatrixa
                                        ,EightMileTest_ConfusionMatrixb
                                        ,EightMileTest_ConfusionMatrixc)
...
write.csv(EightMileTest_ConfusionMatrix_all,"Test_Outputs/2_HDW_imagery/2_Models/EightMileTest_ConfusionMatrix_ALL.csv")
load("Test_Outputs/2_HDW_Imagery/2_Models/rf_HDW_EightMileTest.rda")

#c<-plot(rf_HDW_EightMileTest_n)
#
##min_depth_frame2 <- min_depth_distribution(rf_HDW_EightMileTest_n)
...
Results_HDW    <-predict(rf_HDW_EightMileTest_nc,EightMileTest_data_HDW[-1:-2])
...
write.csv(EightMileTest_HDW_PercentCover ,"Test_Outputs/2_HDW_imagery/2_Models/EightMileTest_HDW_PercentCover_TST3_10.csv")
...
jpeg('Test_Outputs/2_HDW_Imagery/2_Models/EightMileTest Plot Prediction1_50.jpg',width=1200, height=700)
plot(
