...
parse_site_names <- function(names){
  site_names <- vector(mode="character", length = length(names))

  for(i in seq.int(length(names))){
    site_names[[i]] <- names[[i]][[1]]
  }
...
SiteNames <- str_split(RawUID[, 1], "PFT") %>% parse_site_names()
  print(SiteNames)

PFT_IMG_SPEC$Site <- as.character(SiteNames)
PFT_IMG_SPEC <-
  PFT_IMG_SPEC %>%
